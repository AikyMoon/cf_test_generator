...
Формат вывода тестов может быть как по умолчанию, так и определен пользователем. Шаблон состоит из нескольких частей:
1. Номер строки (указывается число в кавычках - особенность так как в json ключ должен быть строкой).
2. Перечисление переменных - список имен переменных
3. Количество повтороений (указать переменную), если есть, в противном случае поставить `null`
4. Разделители - список разделителей, если есть, в противном случае поставить `null`

> [!NOTE]
> `vars` - список переменных (пример: `["a", "b"]`)<br>
> `repeat` - количество повторений или `null`<br>
> `sep` - список разделителей или `null`. При указании `null` разделитель по умолчанию пробел (т.е. данные будут собраны в одну строку)
...
"output_format": {
    "номер_строки": {
        ...
        "repeat": "переменная" или null,
        "sep": список переменных или null
    }
}
```
...
> [!NOTE]
> `min` - минимальное значение переменной <br>
...
> [!WARNING] 
> При использовании данного способа описания длины важно, чтобы переменная была определена раньше, чем последовательность
---
**Указание числа** <br>
...
> [!NOTE]
> При использовании данного способа описания длины задается только число в параметре `len`
...
> [!NOTE]
> При использовании данного способа описания длины задается словарь в параметре `len` с описанием, как у типа данных [Int](#int), т.е. минимальное и максимально значение
...
Перед запуском скрипта необходимо создать файл с конфигурацией и папку, в которую будут выгружаться тесты. После запуска генератор создает подпапку в указанной пользователем папке, а затем генерирует туда `.txt` файлы со следующими названиями:
`название_задачи_номер_подгруппы_номер_сэмпла.txt`.

Перед запуском необходимо в переменные `CONFIG_PATH` и `OUTPUT_DIR` ввести путь к файлу конфигурации и выходной папке соответственно. После запустить скрипт и ожидать генерации файлов

```python
CONFIG_PATH = "путь_к_файлу"
OUTPUT_DIR = "путь_к_папке"
```

>[!NOTE]
> Во время генерации в консоле будет выводиться информация о генерируемых файлах (успешно сгенерирован файл и подгруппа)


<!-- Глава пятая: Контакты для обратной связи -->
# Обратная связь <a id="contacts"></a>
По техническим вопросам писать в телеграмм [@Aiky_Moon](https://t.me/Aiky_Moon) с вопросами/пожеланиями/ошибками и скриншотами ошибок, для ускорения исправления баггов


<!-- Глава шестая: Глоссарий -->
# Глоссарий <a id="glossary"></a>
Список всех переменных и типов данных с их краткими описаниями

---
**Параметры переменных**
1. `min` - минимальное значение переменной (тип [int](#int))
2. `max` - максимальное значение переменной (тип [int](#int))
3. `valid_chars` - список или строка доступных символов (тип [char](#char))
4. `len` - длина последовательности (тип [list](#list))
5. `element_type` - тип данных элемента последовательности (тип [list](#list))
6. `type` - тип данных переменной ([int](#int) / [char](#char) / [list](#list))

---
**Параметры формата вывода**
1. `vars` - список переменных
2. `repeat` - количество повторений (имя переменной или `null`)
3. `sep` - разделитель (список разделителей или `null`)

> [!IMPORTANT]
> В каком порядке задаются переменные в списке `vars`, в таком порядке и будут они выводиться
> <br><br>
> Если разделители задаются списком, то количество разделителей должно быть на один меньше, чем количество переменных, т.е. 
> ```python
> var = ["a", "b", "c"]
> sep = [" ", "\n"]
> Вывод в виде списка: ["a", " ", "b", "\n", "c"]
>```
> Если значение `sep` равно `null`, то разделитель по умолчанию пробел, т.е. все переменные будут выведены через пробел в одну строчку

---
**Типы данных**
1. `int` - случайные числа
2. `char` - случайный символ
3. `list` - последовательности различных типов данных